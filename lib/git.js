...
export const repoIsReadyForPullRequest = (repoInfo) => {
	...
	if (repoInfo.commitDiff.behind > 0) {
		throw new Error(
			`Your branch is behind the default branch by ${repoInfo.commitDiff.behind} commit(s). Please merge the default branch (maybe add how to merge) into your branch before creating a PR.`
		)
	}
